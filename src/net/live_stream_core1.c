...
static bool live_send_session_header(struct tcpserver *server) {
    struct live_session_header_frame frame = {
        .session_id = live_stream_shared.start_response.session_id,
        .selected_sensor_mask = live_stream_shared.start_response.selected_sensor_mask,
        .publish_cadence_ms = live_stream_shared.start_response.publish_cadence_ms,
        ...
        .travel_period_us = live_stream_shared.start_response.travel_period_us,
        .imu_period_us = live_stream_shared.start_response.imu_period_us,
        .gps_fix_interval_ms = live_stream_shared.start_response.gps_fix_interval_ms,
        ...
        .active_imu_count = live_stream_shared.start_response.active_imu_count,
        .active_imu_mask = live_stream_shared.start_response.active_imu_mask,
        .travel_queue_capacity = LIVE_TRAVEL_SLOT_COUNT,
        .imu_queue_capacity = LIVE_IMU_SLOT_COUNT,
        .gps_queue_capacity = LIVE_GPS_SLOT_COUNT,
...
static bool live_send_slot_frame(struct tcpserver *server, uint16_t frame_type, uint32_t stream_type,
    ...
    struct live_batch_payload batch = {
        ...
        .payload_byte_length = header->payload_bytes,
        .queue_depth = header->queue_depth_after_publish,
        .dropped_batches = header->dropped_batches,
    };
...
static bool live_send_next_ready_slot(struct tcpserver *server) {
    ...
    if (slot_index >= 0) {
        ...
        return live_send_slot_frame(server, LIVE_FRAME_TRAVEL_BATCH, LIVE_STREAM_TYPE_TRAVEL, &slot->header,
                                    slot->payload);
    }
    ...
    if (slot_index >= 0) {
        ...
        return live_send_slot_frame(server, LIVE_FRAME_IMU_BATCH, LIVE_STREAM_TYPE_IMU, &slot->header, slot->payload);
    }
    ...
    if (slot_index >= 0) {
        ...
        return live_send_slot_frame(server, LIVE_FRAME_GPS_BATCH, LIVE_STREAM_TYPE_GPS, &slot->header, slot->payload);
    }
